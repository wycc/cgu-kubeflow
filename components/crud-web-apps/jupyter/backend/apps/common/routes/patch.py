...
def set_notebook_resources(namespace, notebook, request_body):
    ...
    if PROXY_PORT_ATTR in request_body:
        if request_body[PROXY_PORT_ATTR]:
            annotations[PROXY_PORT_ANNOTATION] = request_body[PROXY_PORT_ATTR]
        else:
            annotations.pop(PROXY_PORT_ANNOTATION, None)
