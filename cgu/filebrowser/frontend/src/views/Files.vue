<template>
  <div>
    ...
    <breadcrumbs base="/files" :showEdit="showEdit" :name="fileName" :content="fileContent" />
    <errors v-if="error" :errorCode="error.status" />
    <div v-else-if="currentView === 'Notebook'" ref="notebookContainer"></div>
...
const fileName = ref(computed(() => fileStore.req?.name ?? ""));
const fileContent = ref(computed(() => fileStore.req?.content ?? ""));
const showEdit = computed(() => currentView.value === 'Notebook');
...
const currentView = computed(() => {
  ...
  if (fileStore.req.isDir) {
    ...
    fileStore.req.name?.endsWith(".ipynb")
  ){
    return "Notebook"
  }
  else if (
...
watch([currentView, () => fileStore.req], async ([view]) => {
  await nextTick();
  if (view === 'Notebook' && notebookContainer.value && fileStore.req?.content) {
    const content = typeof(fileStore.req.content) === "string" 
      ? JSON.parse(fileStore.req.content) 
