...
映像檔標籤格式：`cguaicadmin/notebook-external-access-controller:<VERSION>`
...
```bash
kubectl get deployment -n cgu notebook-external-access-controller-manager
...
kind: Service
metadata:
  name: my-notebook-external-access-service
  namespace: user-namespace
  ownerReferences:
    - apiVersion: kubeflow.org/v1beta1
      kind: Notebook
...
```
notebook-external-access-controller/
...
image: cguaicadmin/notebook-external-access-controller:1.2.0  # 同步更新此處
```
